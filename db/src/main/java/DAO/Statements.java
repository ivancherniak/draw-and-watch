...
// TODO: 7/29/2019 delete unused strings
...
public class Statements {
    ...
    public static final String GET_PICTURE_BY_ID = "SELECT picture_id, painted_by, (SELECT name FROM users WHERE login = p.login), painted_when_content FROM pictures p WHERE picture_id = ?";
    ...
    public static final String GET_COMMENTS_FOR_PICTURE = "SELECT c.login, (SELECT name FROM users WHERE login = c.login), c.comment_data FROM comments c START WITH parent_id = NULL AND picture_id = ? CONNECT BY PRIOR comment_id = parent_id";
